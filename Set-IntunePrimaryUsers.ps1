...
.COPYRIGHT      (c) 2026 Florian Daminato. MIT License - free to use with attribution.
.TAGS           Intune Graph PrimaryUser DeviceManagement MicrosoftGraph Azure
.LICENSEURI     https://opensource.org/licenses/MIT
.PROJECTURI     https://github.com/fdaminato/Intune-PrimaryUser
.RELEASENOTES
    ...
    7.4.7 Custom - Added live progress logs without changing processing logic
#>
...
if ($ExcludedDeviceNames -and $ExcludedDeviceNames.Count -gt 0) { Write-Host "[FILTER] Excluded device name prefix(es): $($ExcludedDeviceNames -join ', ')" }

try {
    #Sign in to Graph
    try {
        Write-Host "[STEP] Connecting to Microsoft Graph..."
        ...
        Write-Host "[OK] Connected to Microsoft Graph."
    ...
    # Get all devices
    try {
        Write-Host "[STEP] Retrieving Intune managed devices from Microsoft Graph..."
        ...
        if ([string]::IsNullOrWhiteSpace($GraphFilterString)) {
            Write-Host "[QUERY] Device Graph filter: none"
        } else {
            Write-Host "[QUERY] Device Graph filter: $GraphFilterString"
        }
        ...
        if ($AllDevices -and $AllDevices.Count -gt 0) {
            Write-Host "[OK] Retrieved $($AllDevices.Count) devices from Graph API before client-side name filters."
            ...
            if (($IncludedDeviceNames -and $IncludedDeviceNames.Count -gt 0) -or ($ExcludedDeviceNames -and $ExcludedDeviceNames.Count -gt 0)) {
                [int]$DeviceCountBeforeNameFilter = $AllDevices.Count
                Write-Host "[FILTER] Applying device name prefix filters to $DeviceCountBeforeNameFilter devices..."
                ...
                Write-Host "[OK] Device name filters kept $($AllDevices.Count) of $DeviceCountBeforeNameFilter devices."
                Write-Verbose "Remaining after filters: $($AllDevices.Count) devices"
            } else {
                Write-Host "[FILTER] No device name prefix filter applied. Devices to process: $($AllDevices.Count)"
            }
            # Create hashtable for fast lookups
            Write-Host "[STEP] Building device lookup table..."
            $AllDevicesByIdHash = Convert-PSObjectArrayToHashTables -PSObjectArray $AllDevices -IdProperties @('id')
            Write-Host "[OK] Device lookup table created with $($AllDevicesByIdHash.Count) entries."
    ...
    try {
        Write-Host "[STEP] Retrieving users from Microsoft Graph..."
        # List properties to retrieve
        ...
        if ($AllUsers -and $AllUsers.Count -gt 0) {
            Write-Host "[OK] Retrieved $($AllUsers.Count) users from Graph API."
            ...
            Write-Host "[OK] User lookup tables created. ID=$($AllUsersByIdHash.Count), UPN=$($AllUsersByUPNHash.Count)."
    ...
    try {
        Write-Host "[STEP] Retrieving sign-in logs from $($SignInsStartTime.ToString('yyyy-MM-dd HH:mm:ss')) to now..."
        # List properties to retrieve
        ...
        foreach ($OsQuery in $OsQueries) {
            ...
            Write-Host "[STEP] Checking $($OsQuery.Name) sign-in logs..."
            ...
            try {
                ...
                if ($HasResults) {
                    Write-Host "[OK] Found recent $($OsQuery.Name) sign-in logs. Fetching full dataset in chunks..."
                    ...
                    Write-Host "[INFO] $($OsQuery.Name): created $($TimeChunks.Count) sign-in log time chunks."
                    ...
                    foreach ($Chunk in $TimeChunks) {
                        ...
                        Write-Host "[FETCH] $($OsQuery.Name) chunk $ChunkIndex/$($TimeChunks.Count): $($Chunk.StartTime.ToString('yyyy-MM-dd')) to $($Chunk.EndTime.ToString('yyyy-MM-dd'))"
                        ...
                        try {
                            ...
                            if ($ChunkLogs) {
                                ...
                                Write-Host "[OK] $($OsQuery.Name) chunk $ChunkIndex/$($TimeChunks.Count): retrieved $($ChunkLogsArray.Count) sign-in logs."
                            ...
                            Write-Host "[WARN] $($OsQuery.Name) chunk $ChunkIndex/$($TimeChunks.Count) failed. Continuing with next chunk."
                        }
                    }
                    
                    # Process all results for this OS type
                    if ($ChunkResults.Count -gt 0) {
                        Write-Host "[STEP] Processing $($ChunkResults.Count) $($OsQuery.Name) sign-in logs..."
                        ...
                        [array]$FilteredLogs = if ($ReplaceUserAccounts -and $ReplaceUserAccounts.Count -gt 0) {
                            ...
                            Write-Host "[FILTER] $($OsQuery.Name): ReplaceUserAccounts filter kept $($Filtered.Count) of $PreCount sign-in logs."
                        ...
                        Write-Host "[OK] Completed $($OsQuery.Name). Total sign-in logs so far: $($AllSignInLogs.Count). Unique devices with logs: $($SignInsByDeviceHash.Count)."
                        Write-Verbose "Completed $($OsQuery.Name) - Total logs: $($AllSignInLogs.Count), Unique devices: $($SignInsByDeviceHash.Count)"
                    } else {
                        Write-Host "[INFO] $($OsQuery.Name): no sign-in logs returned in the full time range."
                    }
                } else {
                    Write-Host "[INFO] No recent $($OsQuery.Name) sign-in logs found in the 1-day quick check. Full fetch skipped for this OS."
                }
            }
            catch {
                Write-Warning "Failed to retrieve $($OsQuery.Name) sign-in logs: $($_.Exception.Message)"
                Write-Host "[WARN] Failed to retrieve $($OsQuery.Name) sign-in logs. Continuing with next OS query."
            }
        }
        if ($AllSignInLogs -and $AllSignInLogs.Count -gt 0) {
            Write-Host "[OK] Retrieved $($AllSignInLogs.Count) total sign-in logs from $($SignInsByDeviceHash.Count) unique devices."
    ...
    [int]$TotalDevicesToProcess = @($AllDevices).Count
    [int]$DeviceProgressInterval = 25
    Write-Host "[STEP] Processing $TotalDevicesToProcess device(s) to evaluate primary user changes..."
    foreach ($Device in $AllDevices) {
        ...
        if ($TotalDevicesToProcess -gt 0 -and -not $env:AUTOMATION_ASSET_ACCOUNTID) {
            Write-Progress -Activity "Processing Intune primary users" -Status "$ProcessedDeviceCount/$TotalDevicesToProcess - $DeviceName" -PercentComplete ([math]::Round(($ProcessedDeviceCount / $TotalDevicesToProcess) * 100, 0))
        }
        if (($ProcessedDeviceCount -eq 1) -or ($ProcessedDeviceCount % $DeviceProgressInterval -eq 0) -or ($ProcessedDeviceCount -eq $TotalDevicesToProcess)) {
            Write-Host "[PROGRESS] Device $ProcessedDeviceCount/$TotalDevicesToProcess`: $DeviceName"
        }
    ...
    if ($TotalDevicesToProcess -gt 0 -and -not $env:AUTOMATION_ASSET_ACCOUNTID) {
        Write-Progress -Activity "Processing Intune primary users" -Completed
    }
    Write-Host "[OK] Finished processing $ProcessedDeviceCount device(s)."
}
catch {
    Write-Error "Script execution failed: $($_.Exception.Message)"
}
finally { #End Script and restore preferences
    Write-Host "[STEP] Cleaning up Graph connection and generating report if enabled..."
